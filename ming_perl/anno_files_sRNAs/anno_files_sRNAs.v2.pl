...
sub usage_short {
    die("
Usage: perl anno_files_tags.pl -i [tag_dir|tag_dir.list] -o comp_db db_file.txt
...
Options: -i  <STR>          : Support <directory> and a file contains a list of <directory>
                              [dir]  the dir that contains *_sRNA.txt file
                              [file] the file that contains dirs (only one in each line)
                              About the [dir]:
                                  Expect the *_sRNA.txt files in dir, or 'dir/scan_cutoff/update/output';
