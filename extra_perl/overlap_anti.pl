...
sub help{
    print STDERR <<EOF
Usage: perl  overlap.pl  -i  gene.list  -a database.txt  -r 45  -c 0.5

Note:
    i       : The input list should contain at least 6 lines.
              <ID*> <..> <..> <Begin*> <End*> <Strand*>...
    a       : The database file. at least 6 lines.
              <ID*> <..> <..> <Begin*> <End*> <Strand*>...
...
# Set parameters.
my %options = ();
getopt("i:a:r:c:",\%options);
if(!defined $options{i} || !defined $options{a} || !defined $options{c}){
    &help();
    exit(1);
}
$options{r} = (defined $options{r})?$options{r}:1000;
$options{c} = (defined $options{c})?$options{c}:0.5;
